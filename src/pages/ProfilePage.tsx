...
interface Profile { id: number; user_id: string; nickname: string; gender: string; height: number | null; weight: number | null; bio: string | null; }
...
export function ProfilePage() {
  ...
  useEffect(() => {
    if (profile) { setNickname(profile.nickname || ''); setGender(profile.gender || '男'); setHeight(profile.height ? String(profile.height) : ''); setWeight(profile.weight ? String(profile.weight) : ''); setBio(profile.bio || ''); }
  }, [profile]);
  ...
  const handleSave = async () => {
    ...
    const payload = { user_id: user.id, nickname: nickname.trim() || null, gender, height: height ? parseFloat(height) : null, weight: weight ? parseFloat(weight) : null, bio: bio.trim() || null };
    const { error } = profile
