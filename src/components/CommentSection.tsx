...
export function CommentSection({ targetType, targetId }: Props) {
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from('comments').insert({ target_type: targetType, target_id: targetId, user_id: userId, content: content.trim() });
    },
    onSuccess: () => { setContent(''); qc.invalidateQueries({ queryKey: ['comments', targetType, targetId] }); },
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['comments', targetType, targetId] }); toast.success('已删除'); },
  });
            ...
            {displayed.map(c => (
              <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5"><span className="text-xs text-stone-400 font-semibold">{c.user_nickname || '匿名用户'}</span><span className="text-[10px] text-stone-600">{new Date(c.created_at).toLocaleDateString('zh-CN')}</span></div>
